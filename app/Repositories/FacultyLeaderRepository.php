...
class FacultyLeaderRepository extends Repository
{
    ...
    public function getYajraDataTables($additionalParams = null)
    {
        ...
        $editColumns = [
            'currency'         => fn(FacultyLeader $item) => dollar($item->currency),
            'currency_idr'     => fn(FacultyLeader $item) => rp($item->currency_idr),
            ...
            'image'            => fn(FacultyLeader $item) => view('stisla.crud-examples.image', ['file' => $item->image, 'item' => $item]),
            'barcode'          => fn(FacultyLeader $item) => \Milon\Barcode\Facades\DNS1DFacade::getBarcodeHTML($item->barcode, 'C39', 1, 10),
            'qr_code'          => fn(FacultyLeader $item) => \Milon\Barcode\Facades\DNS2DFacade::getBarcodeHTML($item->qr_code, 'QRCODE', 3, 3),
            ...
            // 'created_by'       => fn(FacultyLeader $crudExample) => $crudExample->createdBy ? $crudExample->createdBy->name : '-',
            // 'last_updated_by'  => fn(FacultyLeader $crudExample) => $crudExample->lastUpdatedBy ? $crudExample->lastUpdatedBy->name : '-',
            'action'           => function (FacultyLeader $crudExample) use ($additionalParams) {
        ...
        $params = [
            ...
            'addColumns'  => [
                'created_by' => function (FacultyLeader $item) {
                    return $item->createdBy ? $item->createdBy->name : '-';
                },
                'last_updated_by' => function (FacultyLeader $item) {
