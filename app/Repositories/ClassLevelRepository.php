...
class ClassLevelRepository extends Repository
{
    ...
    public function getYajraDataTables($additionalParams = null)
    {
        ...
        $editColumns = [
            ...
            'level_name' => fn(ClassLevel $item) => $item->level_name,


            // yang ini bisa dikomen aja kalau gak dipakai
            'currency'         => fn(ClassLevel $item) => dollar($item->currency),
            'currency_idr'     => fn(ClassLevel $item) => rp($item->currency_idr),
            ...
            'birthdate'        => fn(ClassLevel $item) => view('stisla.includes.others.item-datetime', ['item' => $item]),
            'email'            => fn(ClassLevel $item) => view('stisla.includes.others.item-email', ['item' => $item]),
            'phone_number'     => fn(ClassLevel $item) => view('stisla.includes.others.item-phonenumber', ['item' => $item]),
            'avatar'           => fn(ClassLevel $item) => view('stisla.includes.others.item-image', ['file' => $item->avatar, 'item' => $item]),
            'image'            => fn(ClassLevel $item) => view('stisla.includes.others.item-image', ['file' => $item->image, 'item' => $item]),
            'barcode'          => fn(ClassLevel $item) => \Milon\Barcode\Facades\DNS1DFacade::getBarcodeHTML($item->barcode, 'C39', 1, 10),
            'qr_code'          => fn(ClassLevel $item) => \Milon\Barcode\Facades\DNS2DFacade::getBarcodeHTML($item->qr_code, 'QRCODE', 3, 3),
            ...
            // 'created_by'       => fn(ClassLevel $classLevel) => $classLevel->createdBy ? $classLevel->createdBy->name : '-',
            // 'last_updated_by'  => fn(ClassLevel $classLevel) => $classLevel->lastUpdatedBy ? $classLevel->lastUpdatedBy->name : '-',

            // yang ini butuh action
            'action'           => function (ClassLevel $classLevel) use ($additionalParams) {
                $isAjaxYajra = Route::is('class-levels.index-ajax-yajra') || request('isAjaxYajra') == 1;
        ...
        $params = [
            ...
            'addColumns'  => [
                'created_by' => function (ClassLevel $item) {
                    return $item->createdBy ? $item->createdBy->name : '-';
                },
                'last_updated_by' => function (ClassLevel $item) {
    ...
    public function getYajraColumns()
    {
        return json_encode([
            ...
            ['data' => 'level_name', 'name' => 'Nama Level'],
