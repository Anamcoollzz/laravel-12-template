...
class SchoolYearRepository extends Repository
{
    ...
    public function getYajraDataTables($additionalParams = null)
    {
        ...
        $editColumns = [
            ...
            'year_name' => fn(SchoolYear $item) => $item->year_name,


            // yang ini bisa dikomen aja kalau gak dipakai
            'currency'         => fn(SchoolYear $item) => dollar($item->currency),
            'currency_idr'     => fn(SchoolYear $item) => rp($item->currency_idr),
            ...
            'birthdate'        => fn(SchoolYear $item) => view('stisla.includes.others.item-datetime', ['item' => $item]),
            'email'            => fn(SchoolYear $item) => view('stisla.includes.others.item-email', ['item' => $item]),
            'phone_number'     => fn(SchoolYear $item) => view('stisla.includes.others.item-phonenumber', ['item' => $item]),
            'avatar'           => fn(SchoolYear $item) => view('stisla.includes.others.item-image', ['file' => $item->avatar, 'item' => $item]),
            'image'            => fn(SchoolYear $item) => view('stisla.includes.others.item-image', ['file' => $item->image, 'item' => $item]),
            'barcode'          => fn(SchoolYear $item) => \Milon\Barcode\Facades\DNS1DFacade::getBarcodeHTML($item->barcode, 'C39', 1, 10),
            'qr_code'          => fn(SchoolYear $item) => \Milon\Barcode\Facades\DNS2DFacade::getBarcodeHTML($item->qr_code, 'QRCODE', 3, 3),
            ...
            // 'created_by'       => fn(SchoolYear $schoolYear) => $schoolYear->createdBy ? $schoolYear->createdBy->name : '-',
            // 'last_updated_by'  => fn(SchoolYear $schoolYear) => $schoolYear->lastUpdatedBy ? $schoolYear->lastUpdatedBy->name : '-',

            // yang ini butuh action
            'action'           => function (SchoolYear $schoolYear) use ($additionalParams) {
                $isAjaxYajra = Route::is('school-years.index-ajax-yajra') || request('isAjaxYajra') == 1;
        ...
        $params = [
            ...
            'addColumns'  => [
                'created_by' => function (SchoolYear $item) {
                    return $item->createdBy ? $item->createdBy->name : '-';
                },
                'last_updated_by' => function (SchoolYear $item) {
    ...
    public function getYajraColumns()
    {
        return json_encode([
            ...
            ['data' => 'year_name', 'name' => 'Tahun Pelajaran'],
