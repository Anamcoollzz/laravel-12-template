...
class OrmawaRepository extends Repository
{
    ...
    public function getYajraDataTables($additionalParams = null)
    {
        ...
        $editColumns = [
            'currency'         => fn(Ormawa $item) => dollar($item->currency),
            'currency_idr'     => fn(Ormawa $item) => rp($item->currency_idr),
            ...
            'image'            => fn(Ormawa $item) => view('stisla.crud-examples.image', ['file' => $item->image, 'item' => $item]),
            'barcode'          => fn(Ormawa $item) => \Milon\Barcode\Facades\DNS1DFacade::getBarcodeHTML($item->barcode, 'C39', 1, 10),
            'qr_code'          => fn(Ormawa $item) => \Milon\Barcode\Facades\DNS2DFacade::getBarcodeHTML($item->qr_code, 'QRCODE', 3, 3),
            ...
            // 'created_by'       => fn(Ormawa $crudExample) => $crudExample->createdBy ? $crudExample->createdBy->name : '-',
            // 'last_updated_by'  => fn(Ormawa $crudExample) => $crudExample->lastUpdatedBy ? $crudExample->lastUpdatedBy->name : '-',
            'action'           => function (Ormawa $crudExample) use ($additionalParams) {
        ...
        $params = [
            ...
            'addColumns'  => [
                'created_by' => function (Ormawa $item) {
                    return $item->createdBy ? $item->createdBy->name : '-';
                },
                'last_updated_by' => function (Ormawa $item) {
