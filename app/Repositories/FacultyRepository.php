...
class FacultyRepository extends Repository
{
    ...
    public function getYajraDataTables($additionalParams = null)
    {
        ...
        $editColumns = [
            'currency'         => fn(Faculty $item) => dollar($item->currency),
            'currency_idr'     => fn(Faculty $item) => rp($item->currency_idr),
            ...
            'image'            => fn(Faculty $item) => view('stisla.crud-examples.image', ['file' => $item->image, 'item' => $item]),
            'barcode'          => fn(Faculty $item) => \Milon\Barcode\Facades\DNS1DFacade::getBarcodeHTML($item->barcode, 'C39', 1, 10),
            'qr_code'          => fn(Faculty $item) => \Milon\Barcode\Facades\DNS2DFacade::getBarcodeHTML($item->qr_code, 'QRCODE', 3, 3),
            ...
            // 'created_by'       => fn(Faculty $crudExample) => $crudExample->createdBy ? $crudExample->createdBy->name : '-',
            // 'last_updated_by'  => fn(Faculty $crudExample) => $crudExample->lastUpdatedBy ? $crudExample->lastUpdatedBy->name : '-',
            'action'           => function (Faculty $crudExample) use ($additionalParams) {
        ...
        $params = [
            ...
            'addColumns'  => [
                'created_by' => function (Faculty $item) {
                    return $item->createdBy ? $item->createdBy->name : '-';
                },
                'last_updated_by' => function (Faculty $item) {
