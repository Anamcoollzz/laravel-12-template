...
class StudyProgramRepository extends Repository
{
    ...
    public function getYajraDataTables($additionalParams = null)
    {
        ...
        $editColumns = [
            'currency'         => fn(StudyProgram $item) => dollar($item->currency),
            'currency_idr'     => fn(StudyProgram $item) => rp($item->currency_idr),
            ...
            'image'            => fn(StudyProgram $item) => view('stisla.crud-examples.image', ['file' => $item->image, 'item' => $item]),
            'barcode'          => fn(StudyProgram $item) => \Milon\Barcode\Facades\DNS1DFacade::getBarcodeHTML($item->barcode, 'C39', 1, 10),
            'qr_code'          => fn(StudyProgram $item) => \Milon\Barcode\Facades\DNS2DFacade::getBarcodeHTML($item->qr_code, 'QRCODE', 3, 3),
            ...
            // 'created_by'       => fn(StudyProgram $crudExample) => $crudExample->createdBy ? $crudExample->createdBy->name : '-',
            // 'last_updated_by'  => fn(StudyProgram $crudExample) => $crudExample->lastUpdatedBy ? $crudExample->lastUpdatedBy->name : '-',
            'action'           => function (StudyProgram $crudExample) use ($additionalParams) {
        ...
        $params = [
            ...
            'addColumns'  => [
                'created_by' => function (StudyProgram $item) {
                    return $item->createdBy ? $item->createdBy->name : '-';
                },
                'last_updated_by' => function (StudyProgram $item) {
