...
class SchoolClassRepository extends Repository
{
    ...
    public function getYajraDataTables($additionalParams = null)
    {
        ...
        $editColumns = [
            ...
            'class_name' => fn(SchoolClass $item) => $item->class_name,


            // yang ini bisa dikomen aja kalau gak dipakai
            'currency'         => fn(SchoolClass $item) => dollar($item->currency),
            'currency_idr'     => fn(SchoolClass $item) => rp($item->currency_idr),
            ...
            'birthdate'        => fn(SchoolClass $item) => view('stisla.includes.others.item-datetime', ['item' => $item]),
            'email'            => fn(SchoolClass $item) => view('stisla.includes.others.item-email', ['item' => $item]),
            'phone_number'     => fn(SchoolClass $item) => view('stisla.includes.others.item-phonenumber', ['item' => $item]),
            'avatar'           => fn(SchoolClass $item) => view('stisla.includes.others.item-image', ['file' => $item->avatar, 'item' => $item]),
            'image'            => fn(SchoolClass $item) => view('stisla.includes.others.item-image', ['file' => $item->image, 'item' => $item]),
            'barcode'          => fn(SchoolClass $item) => \Milon\Barcode\Facades\DNS1DFacade::getBarcodeHTML($item->barcode, 'C39', 1, 10),
            'qr_code'          => fn(SchoolClass $item) => \Milon\Barcode\Facades\DNS2DFacade::getBarcodeHTML($item->qr_code, 'QRCODE', 3, 3),
            ...
            // 'created_by'       => fn(SchoolClass $schoolClass) => $schoolClass->createdBy ? $schoolClass->createdBy->name : '-',
            // 'last_updated_by'  => fn(SchoolClass $schoolClass) => $schoolClass->lastUpdatedBy ? $schoolClass->lastUpdatedBy->name : '-',

            // yang ini butuh action
            'action'           => function (SchoolClass $schoolClass) use ($additionalParams) {
                $isAjaxYajra = Route::is('school-classes.index-ajax-yajra') || request('isAjaxYajra') == 1;
        ...
        $params = [
            ...
            'addColumns'  => [
                'created_by' => function (SchoolClass $item) {
                    return $item->createdBy ? $item->createdBy->name : '-';
                },
                'last_updated_by' => function (SchoolClass $item) {
    ...
    public function getYajraColumns()
    {
        return json_encode([
            ...
            ['data' => 'class_name', 'name' => 'Nama Kelas'],
