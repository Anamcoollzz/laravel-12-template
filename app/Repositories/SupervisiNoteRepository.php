...
use App\Models\SupervisiNote;
use Illuminate\Http\Response;
use Illuminate\Support\Facades\Route;

class SupervisiNoteRepository extends Repository
{
    ...
    public function getYajraDataTables($additionalParams = null)
    {
        ...
        $editColumns = [
            ...
            'notes' => fn(SupervisiNote $item) => $item->notes,


            // yang ini bisa dikomen aja kalau gak dipakai
            'currency'         => fn(SupervisiNote $item) => dollar($item->currency),
            'currency_idr'     => fn(SupervisiNote $item) => rp($item->currency_idr),
            ...
            'birthdate'        => fn(SupervisiNote $item) => view('stisla.includes.others.item-datetime', ['item' => $item]),
            'email'            => fn(SupervisiNote $item) => view('stisla.includes.others.item-email', ['item' => $item]),
            'phone_number'     => fn(SupervisiNote $item) => view('stisla.includes.others.item-phonenumber', ['item' => $item]),
            'avatar'           => fn(SupervisiNote $item) => view('stisla.includes.others.item-image', ['file' => $item->avatar, 'item' => $item]),
            'image'            => fn(SupervisiNote $item) => view('stisla.includes.others.item-image', ['file' => $item->image, 'item' => $item]),
            'barcode'          => fn(SupervisiNote $item) => \Milon\Barcode\Facades\DNS1DFacade::getBarcodeHTML($item->barcode, 'C39', 1, 10),
            'qr_code'          => fn(SupervisiNote $item) => \Milon\Barcode\Facades\DNS2DFacade::getBarcodeHTML($item->qr_code, 'QRCODE', 3, 3),
            ...
            // 'created_by'       => fn(SupervisiNote $supervisiNote) => $supervisiNote->createdBy ? $supervisiNote->createdBy->name : '-',
            // 'last_updated_by'  => fn(SupervisiNote $supervisiNote) => $supervisiNote->lastUpdatedBy ? $supervisiNote->lastUpdatedBy->name : '-',

            // yang ini butuh action
            'action'           => function (SupervisiNote $supervisiNote) use ($additionalParams) {
                $isAjaxYajra = Route::is('supervisi-notes.index-ajax-yajra') || request('isAjaxYajra') == 1;
        ...
        $params = [
            ...
            'addColumns'  => [
                'created_by' => function (SupervisiNote $item) {
                    return $item->createdBy ? $item->createdBy->name : '-';
                },
                'last_updated_by' => function (SupervisiNote $item) {
