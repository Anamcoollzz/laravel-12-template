...
class WorkRepository extends Repository
{
    ...
    public function getYajraDataTables($additionalParams = null)
    {
        ...
        $editColumns = [
            'currency'         => fn(Work $item) => dollar($item->currency),
            'currency_idr'     => fn(Work $item) => rp($item->currency_idr),
            ...
            'image'            => fn(Work $item) => view('stisla.crud-examples.image', ['file' => $item->image, 'item' => $item]),
            'barcode'          => fn(Work $item) => \Milon\Barcode\Facades\DNS1DFacade::getBarcodeHTML($item->barcode, 'C39', 1, 10),
            'qr_code'          => fn(Work $item) => \Milon\Barcode\Facades\DNS2DFacade::getBarcodeHTML($item->qr_code, 'QRCODE', 3, 3),
            ...
            // 'created_by'       => fn(Work $crudExample) => $crudExample->createdBy ? $crudExample->createdBy->name : '-',
            // 'last_updated_by'  => fn(Work $crudExample) => $crudExample->lastUpdatedBy ? $crudExample->lastUpdatedBy->name : '-',
            'action'           => function (Work $crudExample) use ($additionalParams) {
        ...
        $params = [
            ...
            'addColumns'  => [
                'created_by' => function (Work $item) {
                    return $item->createdBy ? $item->createdBy->name : '-';
                },
                'last_updated_by' => function (Work $item) {
