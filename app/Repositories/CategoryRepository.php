...
class CategoryRepository extends Repository
{
    ...
    public function getYajraDataTables($additionalParams = null)
    {
        ...
        $editColumns = [
            ...
            'name' => fn(Category $item) => $item->name,


            // yang ini bisa dikomen aja kalau gak dipakai
            'currency'         => fn(Category $item) => dollar($item->currency),
            'currency_idr'     => fn(Category $item) => rp($item->currency_idr),
            ...
            'birthdate'        => fn(Category $item) => view('stisla.includes.others.item-datetime', ['item' => $item]),
            'email'            => fn(Category $item) => view('stisla.includes.others.item-email', ['item' => $item]),
            'phone_number'     => fn(Category $item) => view('stisla.includes.others.item-phonenumber', ['item' => $item]),
            'avatar'           => fn(Category $item) => view('stisla.includes.others.item-image', ['file' => $item->avatar, 'item' => $item]),
            'image'            => fn(Category $item) => view('stisla.includes.others.item-image', ['file' => $item->image, 'item' => $item]),
            'barcode'          => fn(Category $item) => \Milon\Barcode\Facades\DNS1DFacade::getBarcodeHTML($item->barcode, 'C39', 1, 10),
            'qr_code'          => fn(Category $item) => \Milon\Barcode\Facades\DNS2DFacade::getBarcodeHTML($item->qr_code, 'QRCODE', 3, 3),
            ...
            // 'created_by'       => fn(Category $category) => $category->createdBy ? $category->createdBy->name : '-',
            // 'last_updated_by'  => fn(Category $category) => $category->lastUpdatedBy ? $category->lastUpdatedBy->name : '-',

            // yang ini butuh action
            'action'           => function (Category $category) use ($additionalParams) {
                $isAjaxYajra = Route::is('categories.index-ajax-yajra') || request('isAjaxYajra') == 1;
        ...
        $params = [
            ...
            'addColumns'  => [
                'created_by' => function (Category $item) {
                    return $item->createdBy ? $item->createdBy->name : '-';
                },
                'last_updated_by' => function (Category $item) {
