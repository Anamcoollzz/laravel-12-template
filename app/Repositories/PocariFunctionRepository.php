...
class PocariFunctionRepository extends Repository
{
    ...
    public function getYajraDataTables($additionalParams = null)
    {
        ...
        $editColumns = [
            ...
            'name' => fn(PocariFunction $item) => $item->name,


            // yang ini bisa dikomen aja kalau gak dipakai
            'currency'         => fn(PocariFunction $item) => dollar($item->currency),
            'currency_idr'     => fn(PocariFunction $item) => rp($item->currency_idr),
            ...
            'birthdate'        => fn(PocariFunction $item) => view('stisla.includes.others.item-datetime', ['item' => $item]),
            'email'            => fn(PocariFunction $item) => view('stisla.includes.others.item-email', ['item' => $item]),
            'phone_number'     => fn(PocariFunction $item) => view('stisla.includes.others.item-phonenumber', ['item' => $item]),
            'avatar'           => fn(PocariFunction $item) => view('stisla.includes.others.item-image', ['file' => $item->avatar, 'item' => $item]),
            'image'            => fn(PocariFunction $item) => view('stisla.includes.others.item-image', ['file' => $item->image, 'item' => $item]),
            'barcode'          => fn(PocariFunction $item) => \Milon\Barcode\Facades\DNS1DFacade::getBarcodeHTML($item->barcode, 'C39', 1, 10),
            'qr_code'          => fn(PocariFunction $item) => \Milon\Barcode\Facades\DNS2DFacade::getBarcodeHTML($item->qr_code, 'QRCODE', 3, 3),
            ...
            // 'created_by'       => fn(PocariFunction $pocariFunction) => $pocariFunction->createdBy ? $pocariFunction->createdBy->name : '-',
            // 'last_updated_by'  => fn(PocariFunction $pocariFunction) => $pocariFunction->lastUpdatedBy ? $pocariFunction->lastUpdatedBy->name : '-',

            // yang ini butuh action
            'action'           => function (PocariFunction $pocariFunction) use ($additionalParams) {
                $isAjaxYajra = Route::is('pocari-functions.index-ajax-yajra') || request('isAjaxYajra') == 1;
        ...
        $params = [
            ...
            'addColumns'  => [
                'created_by' => function (PocariFunction $item) {
                    return $item->createdBy ? $item->createdBy->name : '-';
                },
                'last_updated_by' => function (PocariFunction $item) {
