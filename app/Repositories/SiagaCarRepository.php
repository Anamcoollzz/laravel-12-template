...
class SiagaCarRepository extends Repository
{
    ...
    public function getYajraDataTables($additionalParams = null)
    {
        ...
        $editColumns = [
            ...
            'name' => fn(SiagaCar $item) => $item->name,


            // yang ini bisa dikomen aja kalau gak dipakai
            'currency'         => fn(SiagaCar $item) => dollar($item->currency),
            'currency_idr'     => fn(SiagaCar $item) => rp($item->currency_idr),
            ...
            'birthdate'        => fn(SiagaCar $item) => view('stisla.includes.others.item-datetime', ['item' => $item]),
            'email'            => fn(SiagaCar $item) => view('stisla.includes.others.item-email', ['item' => $item]),
            'phone_number'     => fn(SiagaCar $item) => view('stisla.includes.others.item-phonenumber', ['item' => $item]),
            'avatar'           => fn(SiagaCar $item) => view('stisla.includes.others.item-image', ['file' => $item->avatar, 'item' => $item]),
            'image'            => fn(SiagaCar $item) => view('stisla.includes.others.item-image', ['file' => $item->image, 'item' => $item]),
            'barcode'          => fn(SiagaCar $item) => \Milon\Barcode\Facades\DNS1DFacade::getBarcodeHTML($item->barcode, 'C39', 1, 10),
            'qr_code'          => fn(SiagaCar $item) => \Milon\Barcode\Facades\DNS2DFacade::getBarcodeHTML($item->qr_code, 'QRCODE', 3, 3),
            ...
            // 'created_by'       => fn(SiagaCar $siagaCar) => $siagaCar->createdBy ? $siagaCar->createdBy->name : '-',
            // 'last_updated_by'  => fn(SiagaCar $siagaCar) => $siagaCar->lastUpdatedBy ? $siagaCar->lastUpdatedBy->name : '-',

            // yang ini butuh action
            'action'           => function (SiagaCar $siagaCar) use ($additionalParams) {
                $isAjaxYajra = Route::is('siaga-cars.index-ajax-yajra') || request('isAjaxYajra') == 1;
        ...
        $params = [
            ...
            'addColumns'  => [
                'created_by' => function (SiagaCar $item) {
                    return $item->createdBy ? $item->createdBy->name : '-';
                },
                'last_updated_by' => function (SiagaCar $item) {
