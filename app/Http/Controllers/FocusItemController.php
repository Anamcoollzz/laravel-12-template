...
use App\Http\Requests\FocusItemRequest;
use App\Repositories\FocusItemRepository;
use Symfony\Component\HttpFoundation\BinaryFileResponse;

class FocusItemController extends StislaController
{
    ...
    {
        $this->title = 'Focus Item';

        parent::__construct();

        $this->icon         = 'fas fa-cubes';
        $this->repository   = new FocusItemRepository;
        $this->prefix       = $this->viewFolder = 'focus-items';

        // ini sesuaiin sama kebutuhan masing-masing, soalnya kalau A1 kan gede banget
        $this->pdfPaperSize = 'A1';
        $this->isAppCrud    = true;
        $this->request      = new FocusItemRequest;
        $this->fileColumns  = [
            'file',
            'image',
            'avatar',
        ...
        // $this->import     = new FocusItemImport;
    ...
    protected function getStoreData()
    {
        ...
        if ($request->hasFile('file') && in_array('file', $columns))
            $data['file'] = $this->fileUtil->uploadToFolder($request->file('file'), 'focus-items/files');

        if ($request->hasFile('image') && in_array('image', $columns))
            $data['image'] = $this->fileUtil->uploadToFolder($request->file('image'), 'focus-items/images');

        if ($request->hasFile('avatar') && in_array('avatar', $columns))
            $data['avatar'] = $this->fileUtil->uploadToFolder($request->file('avatar'), 'focus-items/avatars');
