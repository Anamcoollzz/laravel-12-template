...
class SemesterController extends StislaController
{
    ...
    public function __construct()
    {
        ...
        $this->prefix       = $this->viewFolder = 'semesters';
        ...
        // $this->import     = new SemesterImport;
    ...
    protected function getStoreData()
    {
        ...
        if ($request->hasFile('file') && in_array('file', $columns))
            $data['file'] = $this->fileUtil->uploadToFolder($request->file('file'), 'semesters/files');

        if ($request->hasFile('image') && in_array('image', $columns))
            $data['image'] = $this->fileUtil->uploadToFolder($request->file('image'), 'semesters/images');

        if ($request->hasFile('avatar') && in_array('avatar', $columns))
            $data['avatar'] = $this->fileUtil->uploadToFolder($request->file('avatar'), 'semesters/avatars');
