...
class SiagaOrderSeeder extends Seeder
{
    /**
    ...
    public function run(): void
    {
        ...
        for ($i = 1; $i <= $total; $i++) {
            ...
            $row['full_name'] = fake()->name();
            $row['phone_number'] = fake()->phoneNumber();
            $row['lokasi_keberangkatan'] = fake()->city();
            $row['alamat_tujuan'] = fake()->city();
            $row['car_type'] = fake()->word();
            $row['tgl_penggunaan'] = fake()->date();
            $row['perkiraan_jam'] = fake()->time();
            $row['additional_notes'] = fake()->sentence();
            $row['status'] = Arr::random(['Menunggu', 'Disetujui', 'Selesai']);
            $rows[] = $row;

            // ✅ Insert per 200/1000 kalau datanya gede. Di contoh ini 20 cukup.
            if (count($rows) >= 200) {
                SiagaOrder::insert($rows);
                $rows = [];
            }
        }
